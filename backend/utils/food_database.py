...
All macro values are per 100 g (standard nutritional reference unit).

To extend:
  • Add entries to FOOD_DB below.
  • For production, replace with USDA FoodData Central API or similar.
"""

from typing import Dict, List, Optional
from backend.schemas.schemas import FoodDBItem

# ── Database ────────────────────────────────────────────────────────────────────
# Sources: USDA FoodData Central averages (rounded to 1 decimal place)
FOOD_DB: Dict[str, FoodDBItem] = {
    ...
    "rice": FoodDBItem(                    # cooked white rice
        name="rice",
    ...
    "tuna": FoodDBItem(                    # canned in water
        name="tuna",
...
def get_food_item(name: str) -> Optional[FoodDBItem]:
    """Case-insensitive lookup. Returns None if food is not in the database."""
    return FOOD_DB.get(name.lower().strip())


def compute_macros(food_name: str, quantity_g: float) -> Optional[Dict[str, float]]:
    """
    Returns a dict of computed macros for `quantity_g` of `food_name`.
    Returns None if the food is unknown (caller should raise HTTP 422).
    """
...
def list_all_foods() -> List[FoodDBItem]:
    """Return every food item in the database, sorted alphabetically."""
