...
Sessions are grouped by date in Python (avoids complex SQL grouping).
"""
...
from backend.models.models import Workout
...
def get_workout_history(
    ...
    """
    Returns workout history grouped by date.
    Each session includes per-exercise detail + total sets and training volume.
    Training volume = Σ(sets × reps × weight_kg) across all exercises that day.
    """
    get_user_or_404(db, user_id)
    ...
    # Group by date (preserves ordering)
...
def delete_workout(db: Session, user_id: int, workout_id: int) -> dict:
    workout = (
        db.query(Workout)
